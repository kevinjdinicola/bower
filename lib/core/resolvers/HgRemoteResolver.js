...
HgRemoteResolver._branches = function (source) {
    ...
    // Not going to cache this here.  This is cached by HgResolver.branches.

    // Lets use the http method of remotely fetching branches.
    var branchRequestUrl = source + '/branches?style=raw';

    // Store the promise in the branches object
    value = promise = Q.nfapply(request, [branchRequestUrl, {
...
HgRemoteResolver._tags = function (source) {
    ...
    // Not going to cache this here.  This is cached by HgResolver.tags.

    // Lets use the http method of remotely fetching tags.
    var tagRequestUrl = source + '/tags?style=raw';

    // Store the promise in the tags object
    value = promise = Q.nfapply(request, [tagRequestUrl, {
            'rejectUnauthorized': false,
            'auth': {
                'user':'',
                'pass':''
